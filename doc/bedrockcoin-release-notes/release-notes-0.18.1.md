BedrockCoin Core version 0.18.1 is now available from:

  <https://download.bedrockcoin.org/bedrockcoin-0.18.1/>
...
`/Applications/BedrockCoin-Qt` (on Mac) or `bedrockcoind`/`bedrockcoin-qt` (on
Linux).
...
BedrockCoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use BedrockCoin Core on unsupported systems.
...
10.10. Additionally, BedrockCoin Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:24731:24731` (this is an extra `:24731` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@bedrockcoin.org) the BedrockCoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 BRK),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in BRK per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/bedrockcoin-conf.md)
  about the `bedrockcoin.conf` file describes how to use it to configure
  BedrockCoin Core.

- A new document introduces BedrockCoin Core's BIP174 [Partially-Signed
  BedrockCoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent BedrockCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  BedrockCoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when BedrockCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of BedrockCoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bedrockcoind with the `-deprecatedrpc=generate`
...
- A new `bedrockcoin-wallet` tool is now distributed alongside BedrockCoin
...
This section describes planned changes to BedrockCoin Core that may affect
other BedrockCoin software and services.

- Since version 0.16.0, BedrockCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with BedrockCoin Core 0.20 (expected about
  a year after 0.18), BedrockCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the BedrockCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in BedrockCoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any BedrockCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, BedrockCoin Core
...
- A sub-project of BedrockCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with BedrockCoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to BedrockCoin Core's own implementation, although entropy
  gathered by BedrockCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves BedrockCoin Core a
...
- On macOS, BedrockCoin Core now opts out of application CPU throttling
...
- #24731 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
