BedrockCoin Core version 0.17.1 is now available from:

  <https://download.bedrockcoin.org/bedrockcoin-0.17.1/>
...
When bedrockcoin is configured with the `-deprecatedrpc=accounts` setting, specifying
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `bedrockcoin.conf` file, not inside included files or from
  ...
  as bedrockcoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the bedrockcoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
The default minimum transaction fee `-mintxfee` has been lowered to 0.0001 BRK/kB after relaxing the minimum relay and dust relay fee rates in prior releases. 
...
prefixing the option with the network, such as:
    ...
    main.uacomment=bedrockcoin
    test.uacomment=bedrockcoin-testnet
...
be used in V0.17 by starting bedrockcoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed BedrockCoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for BedrockCoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://bitcointalk.org/?topic=279249) transactions.

For backend RPC convenience, the BedrockCoin devs have supported to keep the acronym `PSBT`
instead of `PSLT` to make crosschain application support easier.

### Overall workflow

Overall, the construction of a fully signed BedrockCoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid BedrockCoin transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in BRK. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When bedrockcoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If bedrockcoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart bedrockcoind with
...
- When running bedrockcoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If bedrockcoind
