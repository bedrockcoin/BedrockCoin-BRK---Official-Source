...
The BedrockCoin Core daemon can be configured to act as a trusted "border
router", implementing the bedrockcoin wire protocol and relay, making
...
The ZeroMQ feature in BedrockCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bedrockcoind:
...
For instance:
    ...
    $ bedrockcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/bedrockcoind.tx.raw \
...
These options can also be provided in bedrockcoin.conf.
...
From the perspective of bedrockcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bedrockcoind directly. Furthermore, no information is
...
using. BedrockCoind appends an up-counting sequence number to each
