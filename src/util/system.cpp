...
const char * const BITCOIN_CONF_FILENAME = "bedrockcoin.conf";
...
#else
    const char* pszModule = "bedrockcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BedrockCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BedrockCoin";
#else
    // Unix-like
    return pathRet / ".bedrockcoin";
#endif
#endif
}
